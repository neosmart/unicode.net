...
function makeStringArray(keywords) {
	return Enumerable.From(keywords.split(" ")).Where(x => x != "").SelectMany(x => x
			.split("_")).SelectMany(x => x.split("-")).SelectMany(x => x.split(":"))
		.Select(x => x.trim().toLowerCase())
		.Where(x => !(["", "of", "with", "without", "and", "or", "&", "on", "the", "in"]
			.includes(x))).Select(x => sprintf('"%s"', x)).Distinct().ToArray().join(
			", ");
}

function makeSortedSet(name, emoji) {
	result = sprintf(
		'public static readonly SortedSet<SingleEmoji> %s = new SortedSet<SingleEmoji>() {\n',
		name);
...
", CamelCase(emoji.name), emoji.sequence, emoji.name, makeStringArray(emoji.name), emoji
		.index);
}
...
function parseEmoji(data) {
	...
	var i = 0;
	return Lazy(lines).filter(l => !l.startsWith("#") && l.includes("fully-qualified"))
		.map(function(l) {
...
function ungenderedEmoji(emoji) {
	...
	return emoji.filter(e => !e.name.match(/^(man|woman)/i) || !emoji.some(e2 =>
		(e2.name.toLowerCase() == e.name.replace(/^(man|woman)/i, "person")
			.toLowerCase() ||
			e2.name.toLowerCase() == e.name.replace(/^(man|woman) */i, "")
			.toLowerCase())
	));
}
...
loadFont("./seguiemj.ttf", font => {
	...
	$.get("emoji-test.txt", data => {
		...
		var supoprtedEmoji = emoji.filter(e => isSupportedEmoji(font,
			e)); //emoji supported by Segoe UI Emoji
		var basicEmoji = supoprtedEmoji.filter(
			isBasicEmoji); //emoji suitable for display
